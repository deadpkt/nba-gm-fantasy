    ...
    <aside className="cap-space-panel">
      ...
      {player ? (
        ...
        <p className="cap-space-panel__select">
          Select a roster player to review their contract.
        </p>
      )}
      <div className="cap-space-panel__details">
        <div>
          <span>SALARY</span>
          <b>{formatMoney(contract?.salary)}</b>
        </div>
        <div>
          <span>YEARS REMAINING</span>
          <b>{contract ? contract.yearsRemaining : "—"}</b>
        </div>
        <div>
          <span>CONTRACT STATUS</span>
          <b>{status ? status.replace("_", "-") : "NOT INITIALIZED"}</b>
        </div>
      </div>
      <section>
        <span>CONTRACT TIMELINE</span>
        <b>
          {contract ? `SIGNED SEASON ${contract.signedSeason}` : "Unavailable"}
        </b>
        <p>
          Contract years are consumed only when a trusted season enters
          offseason.
        </p>
      </section>
      <section>
        <span>CONTRACT ACCESS</span>
        <b>READ ONLY</b>
        <p>
          Negotiations, extensions, releases, and signings are not available
          yet.
        </p>
      </section>
      <footer>
