...
function PageLayout({ children }) {
  ...
  const showLeagueContext = activeLeague && activeLeague.status !== LEAGUE_STATUS.CANCELLED;
    ...
    <main>
      ...
      {showLeagueContext && (
        <div className="league-context-strip" aria-label="Current league context">
          <Link to={`/league/${activeLeagueId}`}><b>{activeLeague.name}</b><span>SEASON {activeLeague.season}</span></Link>
          <i aria-hidden="true" />
          <span className={`league-phase-badge league-phase-badge--${activeLeague.status}`}>{getLeagueStatusLabel(activeLeague.status)}</span>
        </div>
      )}
      {children}
