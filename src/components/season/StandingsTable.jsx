...
function StandingsTable({
  ...
  const teamByOwner = new Map(
    teams.map((team) => [team.ownerUid || team.id, team]),
  );
  const standings = members
    .map((member) => {
      ...
      return {
        ...
        name: isCurrent
          ? teamName
          : team?.name || member.displayName || "Franchise",
        record: teamRecord,
        percentage: games ? Math.round((teamRecord.wins / games) * 100) : null,
        overall: isCurrent
          ? overall
          : getLineupOverall(team?.roster || [], team?.lineup || {}),
      };
    })
    .sort(
      (first, second) =>
  ...
  return (
    <section className="season-standings">
      <header>
        ...
        <small>Live franchise records</small>
      </header>
      <div className="season-standings__head">
        <span>RK</span>
        <span>TEAM</span>
        <span>W-L</span>
        <span>WIN%</span>
      ...
      {standings.length ? (
        standings.map((team, index) => (
          <div
            className={`season-standings__row ${team.isCurrent ? "is-current" : ""}`}
            key={team.id}
          >
            ...
            <span>
              {team.percentage === null ? "—" : `${team.percentage}%`}
            </span>
            <span>{team.overall || "—"}</span>
            <i>{team.isCurrent ? "CURRENT" : "LEAGUE"}</i>
          </div>
        ))
      ) : (
        <p className="season-standings__empty">
          Join a league to see the season table.
        </p>
      )}
    </section>
  );
