...
function SocialUserRow({ profile, viewerUid, onNavigate }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    void getFollowState(viewerUid, profile.uid)
      .then((value) => {
  ...
  async function toggleFollow() {
    ...
    try {
      await setFollowState(profile.uid, !following);
  ...
  return (
    <article className="social-user-row">
      <Link
      ...
      {!isSelf && (
        <button
          ...
          title={actionError ? "Could not update. Try again." : undefined}
          aria-label={`${following ? "Unfollow" : "Follow"} ${profile.displayName}`}
          onClick={toggleFollow}
        >
          {actionError
            ? "Retry"
            : followActionLabel({ following, pending: loading || busy })}
        </button>
      )}
    </article>
  );
...
function SocialListModal({ uid, type: initialTab, counts = {}, onClose }) {
  ...
  const visibleRows = useMemo(
    () => filterLoadedSocialProfiles(rows, search),
    [rows, search],
  );
  ...
  async function loadPage(reset = false) {
    ...
    setError("");
    try {
      const page = await loadSocialPage({
        uid,
        type: activeTab,
      ...
      setRows((current) =>
        reset ? page.profiles : mergeSocialProfiles(current, page.profiles),
      );
      setCursor(page.cursor);
      setHasMore(page.hasMore);
    } catch (nextError) {
      if (id === requestId.current)
        setError(getUserFriendlyError(nextError, "This list is unavailable."));
    } finally {
      if (id === requestId.current) {
        setLoading(false);
    ...
    >
      ...
      >
        <header>
          <div>
            <span>SOCIAL</span>
            <h2 id="social-modal-title">Connections</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        >
          {SOCIAL_TABS.map((tab) => (
            ...
            >
              <span>{tab}</span>
              <b>{formatSocialCount(counts[`${tab}Count`] || 0)}</b>
            </button>
          ))}
        </div>
        <label className="social-modal__search">
          ...
          <input
            ...
            aria-label="Search loaded people"
          />
        </label>
        <div className="social-modal__list" role="tabpanel">
          ...
          {loading &&
            Array.from({ length: 4 }, (_, index) => (
              <div
                className="social-user-skeleton"
                aria-hidden="true"
                key={index}
              >
                <i />
        ...
        {!loading && !visibleRows.length && !error && (
          <p className="social-modal__empty">
            {search
              ? "No loaded people match that search."
              : socialEmptyMessage(activeTab)}
          </p>
        )}
        {error && (
          <p className="social-modal__error" role="alert">
            {error}
          </p>
        )}
        {hasMore && !loading && (
          <button
            className="social-modal__load button-secondary"
            type="button"
