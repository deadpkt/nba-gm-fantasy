...
function LeagueRouteGuard({ children, requireActive = true, requireMember = true, commissionerOnly = false, allowedStatuses }) {
  ...
  if (access.status === ROUTE_ACCESS.REDIRECT) {
    const messages = {
      ...
      "missing-membership": "Your active league membership is required for that page.",
    ...
    return <Navigate to={access.redirectTo} replace state={{ from: location, leagueAccessMessage: messages[access.reason] }} />;
  }
  return children;
}

export function ActiveLeagueRoute({ children }) { return <LeagueRouteGuard requireMember={false}>{children}</LeagueRouteGuard>; }
export function LeagueMemberRoute({ children }) { return <LeagueRouteGuard>{children}</LeagueRouteGuard>; }
export function CommissionerRoute({ children }) { return <LeagueRouteGuard commissionerOnly>{children}</LeagueRouteGuard>; }
export function LeaguePhaseRoute({ children, statuses }) { return <LeagueRouteGuard allowedStatuses={statuses}>{children}</LeagueRouteGuard>; }
