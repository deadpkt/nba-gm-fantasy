...
export function getPlayerRatings(player) {
  ...
  return {
    ...
    stamina: clamp(
      Math.round(74 + overall * 0.2 + stats.assists * 0.7),
      70,
      99,
    ),
  };
    ...
    events: [
      {
        ...
        type: "tip",
        phase: "early",
        text: "Opening tip: the live match is underway.",
        homeScore: 0,
        awayScore: 0,
      },
    ],
...
export function simulatePossession(game, home, away, random = Math.random) {
  ...
  const clutchFactor =
    next.clock <= 45
      ? (shooterRatings.clutch - defenderRatings.clutch) / 420
      : 0;
  const staminaPenalty =
    (Math.max(0, next.possession - 30) * (100 - shooterRatings.stamina)) /
    25000;
  ...
  if (random() < turnoverChance) {
    ...
    const makeChance = clamp(
      0.39 +
        (shooterRatings.shooting - defenderRatings.defense) / 240 +
        clutchFactor -
        staminaPenalty +
        (isThree ? -0.07 : 0),
      0.25,
    ...
    const blockChance = clamp(
      0.035 + (defenderRatings.defense - shooterRatings.shooting) / 700,
      0.02,
      0.12,
    );
    ...
    if (random() < makeChance) {
      ...
      if (isThree) {
        playerStats[shooter.id].threesMade += 1;
        teamStats.threesMade += 1;
      }
      const assister =
      ...
      const reboundStats = defense.some((player) => player.id === rebounder.id)
        ? opponentStats
        : playerStats;
      const reboundTeamStats = defense.some(
        (player) => player.id === rebounder.id,
      )
        ? opponentTeamStats
        : teamStats;
...
function finishGame(game, home, away) {
  if (game.homeScore === game.awayScore) {
    ...
    game.events.push({
      ...
      type: "clutch",
      phase: "clutch",
      text: `${homeWinsTiebreak ? "Home" : "Away"} converts the game-winning free throw at the buzzer.`,
  ...
  game.mvp =
    allPlayers
      .map((player) => ({ player, stats: stats[player.id] }))
      .sort(
        (a, b) =>
          b.stats.points * 1.2 +
          b.stats.rebounds +
  ...
  game.events.push({
    ...
    type: "final",
    phase: "clutch",
    text: "Final buzzer. The match is complete.",
    homeScore: game.homeScore,
    awayScore: game.awayScore,
  });
}
