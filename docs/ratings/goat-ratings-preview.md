...
npm run fetch:goat-ratings-preview -- --season 2025 --output local-data/ratings/current/goat-ratings-preview-2025.json
```

To stage the generated preview for read-only admin inspection without fetching the provider again:

```powershell
npm run stage:goat-ratings-preview -- --input local-data/ratings/current/goat-ratings-preview-2025.json --confirm --admin-uid FIREBASE_UID
```
