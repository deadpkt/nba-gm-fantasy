...
test("only explicit admins can inspect ratings staging", async () => {
  ...
  await assertSucceeds(getDoc(doc(admin, "playerDataImportReviewHistory/event-1")));
  await assertFails(getDoc(doc(member, "playerDataImportReviewHistory/event-1")));
});
test("all client writes remain blocked, including admins", async () => {
  ...
  await assertFails(setDoc(doc(admin, "playerDataImportReviewHistory/event-2"), { importId: "preview-1" }));
});
