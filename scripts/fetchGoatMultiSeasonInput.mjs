...
async function main() {
  const season = Number(value("--season") || 2025); const input = value("--current-input") || `goat-ratings-preview-${season}-v2.3.json`; const output = value("--output") || `goat-ratings-input-${season - 2}-${season}.json`;
  ...
  const previous = await loadOrFetchSeasonCache({ cachePath: `goat-ratings-cache-${season - 1}.json`, fetchSeason, season: season - 1, logger: console.log });
  const older = await loadOrFetchSeasonCache({ cachePath: `goat-ratings-cache-${season - 2}.json`, fetchSeason, season: season - 2, logger: console.log });
